...
<body <?php echo $c->body; ?>>
    ...
    <?php for ($i=0; $i < count($dat); $i++): ?>
        ...
        　Date： <?php echo $dat[$i]['now'] ?></font>
        <p><blockquote><tt>
        <?php echo preg_replace("/(^|>)(&gt;[^<]*)/i", "\\1<font color=$c->re_color>\\2</font>", $dat[$i]['com']); ?>
        <br></tt>
        <p>
        <?php if (isset($dat[$i]['url']) && !empty($dat[$i]['url'])): ?>
            <a href="http://<?php echo $dat[$i]['url']; ?>" target="_blank">http://<?php echo $dat[$i]['url']; ?></a>
        <?php endif; ?><br>
        <?php echo $dat[$i]['host']; ?></blockquote><p>
    <?php endfor; ?>
    <hr size="1"> <?php echo $start; ?> 番目から <?php echo ($start + count($dat) - 1); ?> 番目の記事を表示<br><center>Page:[<b>
